...
# Reshape the data for SimpleRNN input
X_train = X_train.reshape(X_train.shape[0], 1, X_train.shape[1])
X_test = X_test.reshape(X_test.shape[0], 1, X_test.shape[1])

# Build the SimpleRNN model
model = Sequential()
model.add(SimpleRNN(units=64, input_shape=(1, X.shape[1])))
...
def plotLearningCurve(history,epochs):
  ...
  plt.title('Model Accuracy for RNN', fontsize=23)
  ...
  plt.title('Model Loss for RNN', fontsize=23)
  plt.xlabel('Epoch', fontsize=23)
